...
async def get_recaptcha_v3_token() -> Optional[str]:
    ...
    # Default to headful (False) for better reCAPTCHA reliability
        ...
        async with AsyncCamoufox(headless=headless, main_world_eval=True) as browser:
            ...
            try:
                ...
                for attempt in range(5):
                    ...
                    if "Just a moment" in title or "/cdn-cgi/" in url:
                        ...
                        # Try clicking once just in case
                        if attempt == 0:
                            await click_turnstile(page)
                        break
                    await asyncio.sleep(1)
            ...
            # Check for grecaptcha directly in the main world
            ...
            await safe_page_evaluate(page, "() => { window.__token_result = 'PENDING'; }")
            ...
            for i in range(20): # Wait up to 20 seconds
                ...
                result = await safe_page_evaluate(page, "() => window.__token_result", retries=2)
...
async def get_initial_data():
    ...
    try:
        config = get_config()
        # Default to headful (False) for better Cloudflare/Turnstile reliability
        headless = config.get("camoufox_fetch_headless", False)
        async with AsyncCamoufox(headless=headless, main_world_eval=True) as browser:
            ...
            for i in range(12): # Up to 120 seconds
                ...
                # Check multiple indicators that we're past the challenge
                ...
                try:
                    # Wait for either the title to change OR the URL to change to the main site
                    await page.wait_for_function(
                        "() => document.title.indexOf('Just a moment') === -1 && window.location.href.indexOf('lmarena.ai') !== -1", 
                        timeout=10000
                    )
                    challenge_passed = True
                    break
                except Exception:
                    # Also check if we can find any LMArena-specific elements
                    try:
                        has_content = await page.evaluate("() => document.body.innerText.length > 1000")
                        if has_content:
                            challenge_passed = True
                            break
                    except Exception:
